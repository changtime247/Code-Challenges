...
// Return an array of length equal to the string length.
//      Value of subarray at index i contains digits from 0 to and including i.
// Iterate through string and for each value create an array of digits from 0 to value at corresponding value.

function counterEffect(hitCount) {
  return hitCount.split``.map((v) => {
    let arr = []
    let count = 0
    while (count <= +v) {
      arr.push(count)
