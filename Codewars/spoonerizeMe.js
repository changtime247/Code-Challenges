...
// Swap first letters of the the two words in string

// Given a string consisting of two words separated by a space
// Return a string consisting of the two words separated by a space but with the first letters of swapped
// Ex:
// spoonerize("nit picking"), "pit nicking"
// spoonerize("wedding bells"), "bedding wells"
// spoonerize("jelly beans"), "belly jeans"
// Swap the letters of the words using slice and template literals.

function spoonerize(words) {
  const sw = words.split` `
  return `${sw[1][0]}${sw[0].slice(1)} ${sw[0][0]}${sw[1].slice(1)}`
}
